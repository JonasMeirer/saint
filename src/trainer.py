...
class SaintSupLightningModule(pl.LightningModule):
    ...
    def _shared_step(self, batch, metric_fn):
        ...
        if self.task == 'survival':
            # For survival analysis, targets should contain time and event indicator
            # targets shape: [batch_size, 2] where [:, 0] is time and [:, 1] is event
            loss = self.criterion(outputs, targets[:,1].bool(), targets[:,0])
            with torch.no_grad():
                # For survival metrics, we use the negative risk scores
                # Lower risk score means longer survival
                preds = -outputs
            
            metric_fn.update(preds, targets[:,1].bool(), targets[:,0])
